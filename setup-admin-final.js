...
async function setupAdmin() {
  try {
    ...
    // First, check if permissions tables exist
    try {
      await db.execute(sql`SELECT * FROM permission_groups LIMIT 1;`);
      console.log('✅ Permission tables already exist');
    } catch (error) {
      if (error.message.includes('does not exist')) {
        console.log('Creating permission tables...');
        // Run the permissions migration
        const migrationSql = fs.readFileSync('./migrations/0009_add_permissions_tables.sql', 'utf8');
