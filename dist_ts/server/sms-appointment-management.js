...
export class SMSAppointmentManagementService {
     ...
     */
    async parseManagementRequest(smsText, clientPhone) {
        const text = smsText.toLowerCase().trim();
        // Determine action
        let action = 'reschedule';
     ...
     */
    async findClientAppointments(clientPhone) {
        try {
            ...
            return clientAppointments.sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
        }
        catch (error) {
     ...
     */
    async processManagementRequest(smsText, clientPhone) {
        try {
     ...
     */
    async handleCancellation(request, appointments) {
        ...
        try {
            await this.storage.moveAppointmentToCancelled(appointment.id, request.reason || 'Cancelled via SMS', undefined, 'client');
     ...
     */
    async handleReschedule(request, appointments) {
     ...
     * Find available slots for rescheduling
     */
    async findAvailableSlotsForReschedule(appointment, newDate, newTime) {
        try {
            const service = await this.storage.getService(appointment.serviceId);
            ...
            for (let day = 0; day < 14; day++) {
                ...
                for (const staff of allStaff) {
                    ...
                    const daySchedule = schedules.find(s => s.dayOfWeek.toLowerCase() === format(currentDate, 'EEEE').toLowerCase());
                    if (daySchedule && !daySchedule.isBlocked) {
                        const slots = this.generateTimeSlotsForDay(currentDate, daySchedule.startTime, daySchedule.endTime, service?.duration || 60, staff.id, service, otherAppointments);
     ...
     */
    async performReschedule(appointment, newSlot) {
        try {
            const service = await this.storage.getService(appointment.serviceId);
            const staff = await this.storage.getUser(appointment.staffId);
     ...
     */
    generateTimeSlotsForDay(date, startTime, endTime, duration, staffId, service, existingAppointments) {
        const slots = [];
        ...
        while (slotStart < slotEnd) {
            ...
            const hasConflict = existingAppointments.some(apt => {
                if (apt.staffId !== staffId || apt.status === 'cancelled')
                    return false;
     ...
     */
    parseDateRequest(dateText, currentDate) {
