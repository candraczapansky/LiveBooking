...
export class SMSAppointmentBookingService {
     ...
     */
    async parseBookingRequest(smsText, clientPhone) {
    ...
    async findAvailableSlots(request) {
        ...
        try {
            ...
            if (request.serviceName) {
                targetServices = allServices.filter(service => service.name.toLowerCase().includes(request.serviceName.toLowerCase()));
            }
            // Get all staff
            const allStaff = await this.storage.getAllStaff();
            // Get staff schedules
            const staffSchedules = await Promise.all(allStaff.map(async (staff) => {
            ...
            for (let dayOffset = 0; dayOffset < 7; dayOffset++) {
                ...
                for (const { staff, schedules } of staffSchedules) {
                    const daySchedules = schedules.filter((schedule) => schedule.dayOfWeek === dayName && !schedule.isBlocked);
                    ...
                    for (const staffService of staffServices) {
                        ...
                        for (const schedule of daySchedules) {
                            const slots = this.generateTimeSlots(targetDate, schedule.startTime, schedule.endTime, service.duration, staff.id, service, existingAppointments, staff // Pass staff object for name
                            );
     ...
     */
    generateTimeSlots(date, startTime, endTime, serviceDuration, staffId, service, existingAppointments, staff) {
        const slots = [];
     ...
     * Book an appointment
     */
    async bookAppointment(request, selectedSlot) {
        try {
     ...
     */
    async bookAppointmentStructured(request) {
        try {
     ...
     */
    async findOrCreateClient(phoneNumber, clientName) {
        try {
     ...
     */
    async getSuggestedTimes(request) {
     ...
     */
    async processBookingRequest(smsText, clientPhone) {
        try {
            // Parse the request
            ...
            if (request.date) {
                ...
                if (dateSlots.length > 0) {
                    const timeOptions = dateSlots.slice(0, 5).map(slot => format(slot.startTime, 'h:mm a')).join(', ');
                    return {
                        success: false,
     ...
     */
    async processBookingRequestWithContext(request, clientPhone, conversationState) {
        try {
            ...
            if (currentStep === 'date_selected' && request.time) {
                ...
                if (matchingSlot) {
                    ...
                    const timeOptions = availableSlots.slice(0, 5).map(slot => format(slot.startTime, 'h:mm a')).join(', ');
                    return {
                        success: false,
     ...
     */
    async cancelAppointment(appointmentId, clientPhone) {
        try {
     ...
     */
    async rescheduleAppointment(appointmentId, clientPhone, newDate, newTime) {
        try {
