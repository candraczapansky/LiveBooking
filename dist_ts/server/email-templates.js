...
Handlebars.registerHelper('formatPrice', function (amount) {
    if (!amount)
        return '$0.00';
...
Handlebars.registerHelper('formatDateCT', function (date) {
    if (!date)
        return '';
    const dateObj = typeof date === 'string' ? new Date(date) : date;
...
Handlebars.registerHelper('formatTimeCT', function (date) {
    if (!date)
        return '';
    const dateObj = typeof date === 'string' ? new Date(date) : date;
...
export function generateEmailHTML(template, data, subject) {
...
export function generateEmailText(template, data) {
    const content = template(data);
...
export function generateRawMarketingEmailHTML(editorHtml, unsubscribeUrl) {
