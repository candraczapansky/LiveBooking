...
export function registerNoteTemplateRoutes(app, storage) {
    // Get all note templates
    app.get("/api/note-templates", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const { category, active } = req.query;
        LoggerService.debug("Fetching note templates", { ...context, category, active });
        let templates;
        if (category) {
            templates = await storage.getNoteTemplatesByCategory(category);
        }
        else if (active === "true") {
    ...
    app.get("/api/note-templates/:id", asyncHandler(async (req, res) => {
    ...
    app.post("/api/note-templates", validateRequest(insertNoteTemplateSchema), asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.put("/api/note-templates/:id", validateRequest(updateNoteTemplateSchema), asyncHandler(async (req, res) => {
        const templateId = parseInt(req.params.id);
        const context = getLogContext(req);
        const templateData = req.body;
    ...
    app.delete("/api/note-templates/:id", asyncHandler(async (req, res) => {
        const templateId = parseInt(req.params.id);
        const context = getLogContext(req);
