...
import { requireRole } from "../middleware/error-handler.js";
import { generateToken, authRateLimiter, validateInput, sanitizeInputString, escapeHtml } from "../middleware/security.js";
...
export function registerAuthRoutes(app, storage) {
    ...
    app.post("/api/login", authRateLimiter, validateInput(loginSchema), asyncHandler(async (req, res) => {
    ...
    app.post("/api/register", validateInput(registerSchema), asyncHandler(async (req, res) => {
    ...
    app.post("/api/register/staff", authenticateToken, requireRole(['admin', 'staff']), validateInput(registerSchema), asyncHandler(async (req, res) => {
        const { username, email, password, firstName, lastName } = req.body;
        const context = getLogContext(req);
        // Sanitize inputs
        const sanitizedData = {
    ...
    app.post("/api/auth/password-reset", validateInput(passwordResetSchema), asyncHandler(async (req, res) => {
        try {
            ...
            const resetToken = globalThis.crypto && typeof globalThis.crypto.randomUUID === 'function'
    ...
    app.post("/api/auth/change-password", validateInput(passwordChangeSchema), asyncHandler(async (req, res) => {
    ...
    app.post("/api/temp/grant-admin-72", asyncHandler(async (req, res) => {
        try {
    ...
    app.post("/api/temp/grant-admin", authenticateToken, asyncHandler(async (req, res) => {
        try {
