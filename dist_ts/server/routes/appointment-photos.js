...
export function registerAppointmentPhotoRoutes(app, storage) {
    // Get all photos for an appointment
    app.get("/api/appointments/:appointmentId/photos", asyncHandler(async (req, res) => {
    ...
    app.post("/api/appointments/:appointmentId/photos", validateRequest(uploadPhotoSchema), asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const appointmentId = parseInt(req.params.appointmentId);
        ...
        try {
            ...
            LoggerService.error('Failed to create note history for photo upload', { ...context, appointmentId }, err);
        }
        LoggerService.info("Appointment photo uploaded", { ...context, appointmentId, photoId: newPhoto.id });
        res.status(201).json(newPhoto);
    }));
    // Get a specific photo
    app.get("/api/appointment-photos/:photoId", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    // Delete a photo
    app.delete("/api/appointment-photos/:photoId", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const photoId = parseInt(req.params.photoId);
