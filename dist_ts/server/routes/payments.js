...
import { ValidationError, NotFoundError, ConflictError, ExternalServiceError, asyncHandler } from "../utils/errors.js";
...
async function createSalesHistoryRecord(storage, paymentData, transactionType, additionalData) {
...
export function registerPaymentRoutes(app, storage) {
    // Helcim payment processing is handled in the main routes
    // Create payment
    app.post("/api/payments", validateRequest(insertPaymentSchema), asyncHandler(async (req, res) => {
        ...
        try {
            if (newPayment?.appointmentId && String(newPayment?.status) === 'completed') {
                const apptId = newPayment.appointmentId;
            ...
            try {
                LoggerService.error("Failed to trigger after_payment automation on create", { error: e instanceof Error ? e.message : String(e) });
            }
            catch { }
        }
    ...
    app.get("/api/payments", asyncHandler(async (req, res) => {
        ...
        let payments;
        if (clientId) {
            payments = await storage.getPaymentsByClient(parseInt(clientId));
        }
        else if (staffId) {
    ...
    // Update payment
    app.put("/api/payments/:id", validateRequest(insertPaymentSchema.partial()), asyncHandler(async (req, res) => {
        ...
        try {
            const transitionedToCompleted = String(existingPayment.status) !== 'completed' && String(updatedPayment?.status) === 'completed';
            const apptId = updatedPayment?.appointmentId || existingPayment?.appointmentId;
            ...
            try {
                LoggerService.error("Failed to trigger after_payment automation on update", { ...context, error: e instanceof Error ? e.message : String(e) });
            }
            catch { }
        }
    ...
    app.post("/api/confirm-cash-payment", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        ...
        try {
            ...
            try {
                LoggerService.error("Failed to trigger after_payment automation (cash)", { ...context, paymentId: payment.id, error: e instanceof Error ? e.message : String(e) });
            }
            catch { }
        }
    ...
    app.post("/api/confirm-gift-card-payment", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        ...
        try {
            ...
            try {
                LoggerService.error("Failed to trigger after_payment automation (gift card)", { ...context, paymentId: payment.id, error: e instanceof Error ? e.message : String(e) });
            }
            catch { }
        }
    ...
    // Add gift card
    app.post("/api/add-gift-card", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.get("/api/saved-gift-cards", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const { clientId } = req.query;
        LoggerService.debug("Fetching saved gift cards", { ...context, clientId });
        const giftCards = clientId
            ? await storage.getSavedGiftCardsByClient(parseInt(clientId))
            : await storage.getAllGiftCards();
        res.json(giftCards);
    }));
    // Delete saved gift card
    app.delete("/api/saved-gift-cards/:id", asyncHandler(async (req, res) => {
    ...
    app.post("/api/gift-certificates/purchase", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.get("/api/gift-card-balance/:code", asyncHandler(async (req, res) => {
    ...
    app.post("/api/transactions", asyncHandler(async (req, res) => {
        const body = req.body || {};
        const { clientId, items = [], subtotal = 0, tax = 0, tipAmount = 0, total = 0, paymentMethod = 'card', description = 'POS Sale', helcimPaymentId, helcimTransactionId, cardLast4, terminalPaymentMethod, } = body;
        // Create payment record
        const payment = await storage.createPayment({
        ...
        const productItems = (Array.isArray(items) ? items : []).filter((i) => i?.type === 'product');
        const serviceItems = (Array.isArray(items) ? items : []).filter((i) => i?.type === 'service');
        const productIds = JSON.stringify(productItems.map((i) => i?.item?.id).filter(Boolean));
        const productNames = JSON.stringify(productItems.map((i) => i?.item?.name).filter(Boolean));
        const productQuantities = JSON.stringify(productItems.map((i) => i?.quantity ?? 1));
        const productUnitPrices = JSON.stringify(productItems.map((i) => i?.item?.price ?? 0));
        const serviceIds = JSON.stringify(serviceItems.map((i) => i?.item?.id).filter(Boolean));
        const serviceNames = JSON.stringify(serviceItems.map((i) => i?.item?.name).filter(Boolean));
        await storage.createSalesHistory({
            ...
            serviceIds,
            serviceNames,
            serviceTotalAmount: serviceItems.reduce((sum, i) => sum + (i?.total ?? 0), 0),
            productIds,
            productNames,
            productQuantities,
            productUnitPrices,
            productTotalAmount: productItems.reduce((sum, i) => sum + (i?.total ?? 0), 0),
            membershipId: null,
            membershipName: null,
            ...
            tipAmount: tipAmount || 0,
            discountAmount: 0,
            businessDate: new Date(now.getFullYear(), now.getMonth(), now.getDate()),
            dayOfWeek,
            monthYear,
            quarter,
    ...
    app.post("/api/create-payment", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    // Confirm payment
    app.post("/api/confirm-payment", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        ...
        try {
            const transitionedToCompleted = String(payment.status) !== 'completed' && String(updatedPayment?.status) === 'completed';
            const apptId = appointmentId || payment?.appointmentId;
            if (transitionedToCompleted && apptId) {
            ...
            try {
                LoggerService.error("Failed to trigger after_payment automation (confirm)", { ...context, paymentId, error: e instanceof Error ? e.message : String(e) });
            }
            catch { }
        }
    ...
    app.get("/api/saved-payment-methods", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const { clientId } = req.query;
        LoggerService.debug("Fetching saved payment methods", { ...context, clientId });
        const paymentMethods = clientId
            ? await storage.getSavedPaymentMethodsByClient(parseInt(clientId))
            : await storage.getSavedPaymentMethodsByClient(0); // Get all by using 0 as clientId
        res.json(paymentMethods);
    }));
    // Save payment method
    app.post("/api/saved-payment-methods", validateRequest(insertSavedPaymentMethodSchema), asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.delete("/api/saved-payment-methods/:id", asyncHandler(async (req, res) => {
    ...
    app.put("/api/saved-payment-methods/:id/default", asyncHandler(async (req, res) => {
        const paymentMethodId = parseInt(req.params.id);
        const context = getLogContext(req);
    ...
    // Get all gift cards
    app.get("/api/gift-cards", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
