...
export default function createHelcimWebhookRoutes(storage) {
    const router = Router();
    ...
    const handler = async (req, res) => {
        try {
            ...
            let maybe = body?.payload ?? body?.data ?? body?.event ?? body;
            ...
            try {
                setImmediate(async () => {
                    try {
                        ...
                        try {
                            console.error('❌ Async helcim webhook processing error:', err);
                        }
                        catch { }
                    }
                });
    ...
    router.post('/webhook/success', (req, res) => {
        try {
            ...
            try {
                storage.__terminalService?.handleWebhook?.({ id, transactionId: id, type: 'cardTransaction', approved: true });
            }
            catch { }
