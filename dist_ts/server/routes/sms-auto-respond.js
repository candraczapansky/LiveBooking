...
export function registerSmsAutoRespondRoutes(app, storage) {
    ...
    async function setSmsOptOut(phone) {
        try {
    ...
    async function clearSmsOptOut(phone) {
        try {
            const key = `sms_opt_out:${normalizePhoneForKey(phone)}`;
    ...
    async function updateUserSmsFlagsByPhone(phone, enabled) {
        try {
            ...
            if (!user) {
                ...
                if (digits.length >= 10) {
                    ...
                    user = all.find(u => (u.phone || '').replace(/\D/g, '').slice(-10) === last10);
                }
            }
            if (!user || !user.id)
                return;
    ...
    // Health
    app.get('/api/sms-auto-respond/health', async (_req, res) => {
        try {
    ...
    // Configuration
    app.get('/api/sms-auto-respond/config', (_req, res) => {
        try {
    ...
    app.put('/api/sms-auto-respond/config', async (req, res) => {
        try {
            ...
            await smsService.updateConfig(mergedPayload);
    ...
    // Stats
    app.get('/api/sms-auto-respond/stats', async (_req, res) => {
        try {
    ...
    app.get('/api/sms-auto-respond/conversation-flows', (_req, res) => {
        try {
            const flowsMap = smsService.conversationFlows;
    ...
    app.post('/api/sms-auto-respond/conversation-flows', (req, res) => {
        try {
            const schema = z.object({
    ...
    app.put('/api/sms-auto-respond/conversation-flows', (req, res) => {
        try {
            const schema = z.object({
                id: z.string().min(1),
                name: z.string().min(1),
    ...
    app.delete('/api/sms-auto-respond/conversation-flows/:id', (req, res) => {
        try {
    ...
    app.post('/api/sms-auto-respond/webhook', async (req, res) => {
        try {
    ...
    app.get('/api/sms-auto-respond/webhook', (_req, res) => {
