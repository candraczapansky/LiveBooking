let config = { openai: { apiKey: '', model: 'gpt-3.5-turbo', maxTokens: 500, temperature: 0.7 } };
let DatabaseConfig;
try {
    const mod = await import('./config.js');
    config = mod.config || config;
    DatabaseConfig = mod.DatabaseConfig || (class {
        constructor(_s) { }
        async getOpenAIKey() { return process.env.OPENAI_API_KEY || null; }
    });
}
catch {
...
export class LLMService {
    ...
    async generateResponse(clientMessage, context, channel = 'email') {
        try {
     ...
     */
    async generateStructuredBookingResponse(clientMessage, context, conversationState) {
        try {
    ...
    buildSystemPrompt(context, channel) {
...
- For FAQ questions: Use the business knowledge above to provide comprehensive answers`;
        ...
        if (context.businessKnowledge && context.businessKnowledge.length > 0) {
            ...
            context.businessKnowledge.forEach((item) => {
                prompt += `\nQ: ${item.title}\nA: ${item.content}\n`;
            });
...
[ACTION: escalate: Transfer to human staff: {"reason": "complex inquiry", "urgency": "medium"}]`;
     ...
     */
    buildStructuredBookingPrompt(context, conversationState) {
        const businessName = context.businessName || 'Glo Head Spa';
        const businessType = context.businessType || 'salon and spa';
...
- Do NOT ask for confirmation or more details - just call the function`;
     ...
     */
    buildStructuredUserPrompt(clientMessage, conversationState) {
        ...
        const isContinuingConversation = continuingConversationKeywords.some(keyword => clientMessage.toLowerCase().includes(keyword));
     ...
     */
    getNextStepInstruction(conversationState) {
        const hasService = conversationState?.selectedService;
        const hasDate = conversationState?.selectedDate;
        const hasTime = conversationState?.selectedTime;
    ...
    buildUserPrompt(clientMessage, context, channel) {
        let prompt = `Client Message: "${clientMessage}"`;
        ...
        if (context.conversationHistory && context.conversationHistory.length > 0) {
            ...
            const isSimpleGreeting = simpleGreetings.some(greeting => clientMessage.toLowerCase().trim() === greeting);
            if (isSimpleGreeting) {
...
- Just be friendly and welcoming`;
    ...
    async callOpenAI(systemPrompt, userPrompt, functions) {
    ...
    shouldUseResponsesAPI() {
        try {
            ...
            if (envOverride === 'true' || envOverride === '1' || envOverride === 'yes')
                return true;
    ...
    async callOpenAIChatCompletions(systemPrompt, userPrompt, functions) {
        try {
    ...
    async callOpenAIResponses(systemPrompt, userPrompt, functions) {
        try {
            const apiKey = await this.ensureApiKey();
            if (!apiKey) {
                console.error('LLM Service: OpenAI API key not configured');
            ...
            console.log('LLM Service: Model:', this.config.model);
            console.log('LLM Service: Max tokens:', this.config.maxTokens);
            const tools = functions && functions.length > 0 ? functions.map((fn) => ({
                type: 'function',
                function: {
                    name: fn.name,
    ...
    parseLLMResponse(response) {
        ...
        if (cleanMessage.length > 50)
            confidence += 0.05;
        if (cleanMessage.includes('appointment') || cleanMessage.includes('book'))
            confidence += 0.05;
        return {
     ...
     */
    parseStructuredBookingResponse(response) {
        const actions = [];
    ...
    async saveConversation(clientId, message, response, channel, metadata) {
        try {
