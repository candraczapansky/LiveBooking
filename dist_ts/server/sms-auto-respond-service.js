...
export class SMSAutoRespondService {
    ...
    static getInstance(storage) {
        if (!global.smsAutoRespondService) {
            global.smsAutoRespondService = new SMSAutoRespondService(storage);
        }
        return global.smsAutoRespondService;
    }
    static clearInstance() {
        if (global.smsAutoRespondService) {
            delete global.smsAutoRespondService;
    ...
    validateTimeFormat(time) {
        if (!time)
            return { isValid: false, reason: 'No time provided' };
     ...
     */
    getMessageIntent(smsText, phoneNumber) {
     ...
     */
    isAppointmentBookingRequest(smsText, phoneNumber) {
        const text = smsText.toLowerCase().trim();
     ...
     */
    async findOrCreateClient(phoneNumber) {
        try {
            // Try to find existing client by phone number
            const users = await this.storage.getAllUsers();
            let client = users.find((u) => u.phone && u.phone.replace(/\D/g, '') === phoneNumber.replace(/\D/g, ''));
            if (!client) {
     ...
     */
    async buildContext(client, sms) {
        try {
            // Get recent appointments for context
            const appointments = await this.storage.getAllAppointments();
            const clientAppointments = appointments.filter((apt) => apt.client_id === client.id).slice(-5); // Last 5 appointments
     ...
     */
    async buildEnhancedContext(client, sms) {
        try {
            ...
            const clientAppointments = existingAppointments.filter((apt) => apt.client_id === client.id).slice(-10); // Last 10 appointments for better context
            ...
            const staffWithSchedules = await Promise.all(staff.map(async (s) => {
            ...
            return {
                ...
                client: {
                    ...
                    recent_appointments: clientAppointments.map((apt) => ({
                        id: apt.id,
     ...
     */
    async generateAvailabilityData(services, staffWithSchedules, upcomingAppointments) {
        const today = new Date();
        const next7Days = [];
        let totalSlots = 0;
        const popularTimes = {};
        ...
        for (let i = 0; i < 7; i++) {
            ...
            for (const staff of staffWithSchedules) {
                const daySchedules = staff.schedules.filter((schedule) => schedule.dayOfWeek === dayName && !schedule.isBlocked);
     ...
     */
    categorizeBusinessKnowledge(businessKnowledge) {
        const categorized = {};
     ...
     */
    isExcludedPhoneNumber(phoneNumber) {
        const normalizedPhone = phoneNumber.replace(/\D/g, '');
        return this.config.excludedPhoneNumbers.some(excluded => excluded.replace(/\D/g, '') === normalizedPhone);
    }
    /**
     * Check if SMS is sent to an auto-respond phone number
     */
    isAutoRespondPhoneNumber(phoneNumber) {
        if (this.config.autoRespondPhoneNumbers.length === 0) {
            return true; // If no specific numbers configured, respond to all
        }
        const normalizedPhone = phoneNumber.replace(/\D/g, '');
        return this.config.autoRespondPhoneNumbers.some(respondTo => respondTo.replace(/\D/g, '') === normalizedPhone);
    }
    /**
     ...
     */
    async updateAutoRespondPhoneNumbers(phoneNumbers) {
     ...
     */
    generateFallbackResponse(smsBody, context) {
     ...
     */
    async sendFallbackResponse(sms, response, client) {
        try {
     ...
     */
    async logAutoResponse(sms, response, confidence, client) {
        try {
     ...
     */
    async executeConversationFlow(flowId, sms, client, parsedRequest, conversationState) {
        ...
        let currentStep = flow.steps.find(step => {
            if (step.type === 'condition') {
                if (step.content === 'hasService' && !hasService)
                    return true;
                if (step.content === 'hasDate' && hasService && !hasDate)
                    return true;
                if (step.content === 'hasTime' && hasService && hasDate && !hasTime)
                    return true;
            }
            return false;
        });
    ...
    async executeResponseStep(step, sms, client, parsedRequest, conversationState) {
    ...
    // Execute action step
    async executeActionStep(step, sms, client, parsedRequest, conversationState) {
    ...
    async showAvailableTimes(sms, client, parsedRequest, conversationState) {
        try {
            ...
            const bookingResult = await this.appointmentBookingService.processBookingRequestWithContext(enhancedRequest, sms.from, this.getConversationState(sms.from));
    ...
    // Book appointment
    async bookAppointment(sms, client, parsedRequest, conversationState) {
        try {
            const enhancedRequest = {
    ...
    // Fallback handler
    async handleBookingRequestFallback(sms, client) {
     ...
     */
    async handleBookingRequest(sms, client) {
        try {
            // Get current conversation state
            const conversationState = this.getConversationState(sms.from);
     ...
     */
    async handleStructuredBookingFlow(sms, client, conversationState) {
        try {
            ...
            const llmResponse = await this.llmService.generateStructuredBookingResponse(sms.body, context, conversationState);
     ...
     */
    async handleCheckAvailability(sms, client, args) {
        try {
     ...
     */
    async handleBookAppointment(sms, client, args) {
        try {
     ...
     */
    async handleCancelAppointment(sms, client, args) {
        try {
            console.log('📞 Cancelling appointment with args:', args);
            const cancelResult = await this.appointmentBookingService.cancelAppointment(args.appointment_id, args.client_phone);
     ...
     */
    async handleRescheduleAppointment(sms, client, args) {
        try {
            console.log('📞 Rescheduling appointment with args:', args);
            const rescheduleResult = await this.appointmentBookingService.rescheduleAppointment(args.appointment_id, args.client_phone, args.new_date, args.new_time);
     ...
     */
    async handleRescheduleRequest(sms, client) {
     ...
     */
    async handleCancelRequest(sms, client) {
     ...
     */
    async handleBusinessQuestion(sms, client) {
        try {
     ...
     */
    async handleBusinessQuestionFallback(sms, client, context) {
        ...
        if (text.includes('services') || text.includes('what do you offer') || text.includes('do you offer')) {
            const serviceList = services.map((s) => `• ${s.name} - $${s.price} (${s.duration} minutes)`).join('\n');
        ...
        if (text.includes('haircut') || text.includes('hair cut') || text.includes('hair styling')) {
            const haircutServices = services.filter((s) => s.name.toLowerCase().includes('haircut'));
            const serviceList = haircutServices.map((s) => `• ${s.name} - $${s.price} (${s.duration} minutes)`).join('\n');
     ...
     */
    async handleGeneralMessage(sms, client) {
        const text = sms.body.toLowerCase().trim();
        const customerName = client.firstName || 'there';
        ...
        if (confusionKeywords.some(keyword => text.includes(keyword))) {
            ...
            try {
                const currentServices = await this.storage.getAllServices();
                serviceList = currentServices.map((service) => `• ${service.name} - $${service.price} (${service.duration} minutes)`).join('\n');
            }
            catch (error) {
     ...
     * Send SMS response
     */
    async sendSMSResponse(sms, response, confidence, client) {
        try {
