...
function ensureSmsCompliance(message) {
...
function normalizePhoneForKey(phone) {
    try {
        ...
        if (digitsOnly.length > 0)
            return `+${digitsOnly}`; // fallback international
    }
    catch { }
    return (phone || '').toString();
}
// Check if a phone number is globally opted out via system configuration
async function isPhoneGloballyOptedOut(phone) {
    try {
        if (!storage)
            return false; // gracefully skip if DB unavailable
...
async function markPhoneOptedOut(phone, reason = 'twilio_block_21610') {
    try {
        if (!storage)
            return;
        ...
        if (existing) {
            ...
            await storage.setSystemConfig({ key, value, description: 'Auto SMS opt-out', isEncrypted: false, isActive: true });
        }
...
export async function sendSMS(to, message, photoUrl) {
    ...
    if (process.env.NODE_ENV === 'development') {
        ...
        const isExplicitTestNumber = lowerTo.includes('test') ||
...
export async function sendMMS(to, message, photoUrl) {
    // Ensure Twilio client is initialized
    if (!twilioClient) {
        await initializeTwilioClient();
    }
    ...
    try {
        ...
        const urlHost = (() => {
            try {
                return isHttpUrl ? new URL(photoUrl).hostname : '';
            }
            catch {
                return '';
            }
        ...
        const isHttps = (() => { try {
            return new URL(photoUrl).protocol === 'https:';
        }
        catch {
            return false;
        } })();
