...
async function seedPermissions() {
    try {
        ...
        for (const categoryName of categories) {
            ...
            const result = await db.execute(sql `INSERT INTO permission_categories (name, description) VALUES (${categoryName}, ${`Permissions for ${categoryName}`}) ON CONFLICT (name) DO NOTHING RETURNING id, name`);
            ...
            if (rows && rows.length > 0) {
                ...
                const existing = await db.execute(sql `SELECT id FROM permission_categories WHERE name = ${categoryName}`);
        ...
        for (const permission of DEFAULT_PERMISSIONS) {
            ...
            const result = await db.execute(sql `INSERT INTO permissions (name, description, category_id, is_active) VALUES (${permission.name}, ${permission.description}, ${categoryId}, true) ON CONFLICT (name) DO NOTHING RETURNING id, name`);
            const rows = result?.rows ?? result;
            if (rows && rows.length > 0) {
                ...
                const existing = await db.execute(sql `SELECT id FROM permissions WHERE name = ${permission.name}`);
                const existingRows = existing?.rows ?? existing;
                if (existingRows && existingRows.length > 0) {
        ...
        for (const group of DEFAULT_PERMISSION_GROUPS) {
            ...
            const result = await db.execute(sql `INSERT INTO permission_groups (name, description, is_active) VALUES (${group.name}, ${group.description}, true) ON CONFLICT (name) DO NOTHING RETURNING id, name`);
            const rows = result?.rows ?? result;
            let groupId;
            if (rows && rows.length > 0) {
                groupId = rows[0].id;
                ...
                const existing = await db.execute(sql `SELECT id FROM permission_groups WHERE name = ${group.name}`);
                const existingRows = existing?.rows ?? existing;
                if (existingRows && existingRows.length > 0) {
            ...
            if (groupId) {
                ...
                for (const permissionName of group.permissions) {
                    ...
                    if (permissionId) {
                        await db.execute(sql `INSERT INTO permission_group_mappings (group_id, permission_id) VALUES (${groupId}, ${permissionId}) ON CONFLICT (group_id, permission_id) DO NOTHING`);
        ...
        // Print summary
        const permissionCountRes = await db.execute(sql `SELECT COUNT(*) as count FROM permissions`);
        const groupCountRes = await db.execute(sql `SELECT COUNT(*) as count FROM permission_groups`);
        const categoryCountRes = await db.execute(sql `SELECT COUNT(*) as count FROM permission_categories`);
