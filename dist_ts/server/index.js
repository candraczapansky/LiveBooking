...
app.use((req, res, next) => {
    ...
    const isAllowed = !!origin && (origin.includes('.replit.dev') ||
...
try {
    (async () => {
        try {
            ...
            log(`Security middleware not loaded: ${e?.message || e}`);
        }
    })();
}
catch { }
...
(async () => {
    try {
        ...
        const server = await routesModule.registerRoutes(app, storage);
        ...
        try {
            ...
            if (bookingOnlyHosts.length > 0) {
                app.use((req, res, next) => {
                    ...
                    if (!bookingOnlyHosts.includes(hostHeader))
                        return next();
        ...
        if (app.get("env") === "development") {
            try {
                ...
                log(`Vite dev server not available: ${e?.message || e}`);
            }
        }
        else {
