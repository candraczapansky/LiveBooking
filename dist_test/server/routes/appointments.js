...
import { NotFoundError, ConflictError, asyncHandler } from "../utils/errors.js";
...
export function registerAppointmentRoutes(app, storage) {
    // TEST ENDPOINT: Check SendGrid configuration
    app.get("/api/test-sendgrid-config", asyncHandler(async (req, res) => {
    ...
    app.get("/api/test-confirmation", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.get("/api/appointments", asyncHandler(async (req, res) => {
        ...
        let appointments;
        if (staffId) {
            appointments = await storage.getAppointmentsByStaff(parseInt(staffId));
        }
        else if (clientId) {
            appointments = await storage.getAppointmentsByClient(parseInt(clientId));
        }
        else if (date) {
            ...
            appointments = await storage.getAppointmentsByLocation(parseInt(locationId));
    ...
    app.get("/api/appointments/active", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const { staffId, date } = req.query;
        LoggerService.debug("Fetching active appointments", { ...context, filters: { staffId, date } });
        let appointments;
        if (staffId) {
            appointments = await storage.getActiveAppointmentsByStaff(parseInt(staffId));
        }
        else if (date) {
            appointments = await storage.getActiveAppointmentsByDate(new Date(date));
        }
        else {
            // For general active appointments, filter from all appointments
            const allAppointments = await storage.getAllAppointments();
            appointments = allAppointments.filter(apt => apt.status === "pending" || apt.status === "confirmed" || apt.status === "completed");
        }
        LoggerService.info("Active appointments fetched", { ...context, count: appointments.length });
        res.json(appointments);
    }));
    // Create new appointment
    app.post("/api/appointments", validateRequest(insertAppointmentSchema), asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        ...
        const serviceIdToRoomId = new Map(allServices.map((svc) => [svc.id, svc.roomId ?? null]));
        ...
        if (newAppointmentRoomId != null) {
            const roomInfo = Array.isArray(allRooms) ? allRooms.find((r) => r.id === newAppointmentRoomId) : undefined;
            const capacity = Number(roomInfo?.capacity ?? 1) || 1;
            // Count overlapping appointments in the same room (active statuses only)
            const newStart = new Date(appointmentData.startTime);
            const newEnd = new Date(appointmentData.endTime);
            const overlappingInRoom = allAppointments.filter((apt) => {
                const existingRoomId = serviceIdToRoomId.get(apt.serviceId) ?? null;
                if (!(existingRoomId != null && existingRoomId === newAppointmentRoomId))
                    return false;
        ...
        try {
            const addOnServiceIds = Array.isArray(req.body.addOnServiceIds)
                ? req.body.addOnServiceIds.map((n) => parseInt(n))
                : [];
        ...
        try {
            ...
            if (client && staff && service) {
                ...
                if (client.emailAppointmentReminders && client.email) {
                    try {
                        ...
                        const emailRule = Array.isArray(rules) ? rules.find((r) => r.active && r.type === 'email' && r.trigger === 'booking_confirmation') : null;
                        if (emailRule) {
                ...
                `
                        ...
                        LoggerService.error("Failed to send email confirmation", { ...context, appointmentId: newAppointment.id }, emailError);
                    }
                }
                else {
            ...
            if (client && client.smsAppointmentReminders && client.phone) {
                ...
                try {
                    ...
                    try {
                        const rules = await storage.getAllAutomationRules();
                        const smsRule = Array.isArray(rules) ? rules.find((r) => r.active && r.type === 'sms' && r.trigger === 'booking_confirmation') : null;
                        if (smsRule) {
                            const apptStart = new Date(newAppointment.startTime);
                            const dateStr = new Intl.DateTimeFormat('en-US', { timeZone: 'America/Chicago', month: 'long', day: 'numeric', year: 'numeric' }).format(apptStart);
                            const timeStr = new Intl.DateTimeFormat('en-US', { timeZone: 'America/Chicago', hour: 'numeric', minute: '2-digit', hour12: true }).format(apptStart);
                    ...
                    LoggerService.error("Failed to send SMS confirmation", { ...context, appointmentId: newAppointment.id }, smsError);
                }
            }
            else {
            ...
            LoggerService.error("Failed to send appointment confirmation", { ...context, appointmentId: newAppointment.id }, error);
    ...
    app.post("/api/appointments/:id/cancel", asyncHandler(async (req, res) => {
        const appointmentId = parseInt(req.params.id);
        const context = getLogContext(req);
        ...
        const cancelledByRole = req.user?.role || 'system';
        ...
        try {
            ...
            LoggerService.error("Failed to trigger cancellation automation", { ...context, appointmentId }, autoErr);
        }
        LoggerService.logAppointment("cancelled", appointmentId, context);
        res.json({ success: true, message: "Appointment cancelled successfully" });
    }));
    // Get appointments by client
    app.get("/api/appointments/client/:clientId", asyncHandler(async (req, res) => {
    ...
    // Update appointment
    app.put("/api/appointments/:id", validateRequest(insertAppointmentSchema.partial()), asyncHandler(async (req, res) => {
        const appointmentId = parseInt(req.params.id);
        const context = getLogContext(req);
        ...
        if (updateData.startTime || updateData.endTime || updateData.staffId || updateData.serviceId || updateData.locationId) {
            ...
            const effectiveLocationId = updateData.locationId ?? existingAppointment.locationId;
    ...
    app.delete("/api/appointments/:id", asyncHandler(async (req, res) => {
        ...
        try {
            ...
            LoggerService.error("Failed to trigger cancellation automation from delete", { ...context, appointmentId }, autoErr);
        }
    ...
    app.get("/api/cancelled-appointments", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const { clientId, staffId } = req.query;
        LoggerService.debug("Fetching cancelled appointments", { ...context, filters: { clientId, staffId } });
        let appointments;
        if (clientId) {
            appointments = await storage.getCancelledAppointmentsByClient(parseInt(clientId));
        }
        else if (staffId) {
            appointments = await storage.getCancelledAppointmentsByStaff(parseInt(staffId));
        }
        else {
    ...
    app.get("/api/appointment-history", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const { appointmentId } = req.query;
        LoggerService.debug("Fetching appointment history", { ...context, appointmentId });
        const history = await storage.getAppointmentHistory(parseInt(appointmentId));
        res.json(history);
    }));
    // Create appointment history entry
    app.post("/api/appointment-history", validateRequest(insertAppointmentHistorySchema), asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.post("/api/appointments/:id/send-reminder", asyncHandler(async (req, res) => {
        const appointmentId = parseInt(req.params.id);
        const context = getLogContext(req);
        ...
        if (client.emailAppointmentReminders && client.email) {
          ...
          `
                ...
                LoggerService.error("Failed to send email reminder", { ...context, userId: client.id }, error);
            }
        }
        // Send SMS reminder
        if (client.smsAppointmentReminders && client.phone) {
            try {
                ...
                LoggerService.error("Failed to send SMS reminder", { ...context, userId: client.id }, error);
            }
        }
        if (reminderSent) {
    ...
    app.post("/api/appointments/send-daily-reminders", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        ...
        for (const appointment of appointments) {
            try {
              ...
              `
                        ...
                        LoggerService.error("Failed to send email reminder", { ...context, userId: client.id, appointmentId: appointment.id }, error);
                    }
                }
                // Send SMS reminder
                if (client.smsAppointmentReminders && client.phone) {
                    try {
                        ...
                        LoggerService.error("Failed to send SMS reminder", { ...context, userId: client.id, appointmentId: appointment.id }, error);
                    }
                }
                if (reminderSent) {
                    sentCount++;
                ...
                errorCount++;
                LoggerService.error("Error processing appointment reminder", { ...context, appointmentId: appointment.id }, error);
            }
        }
    ...
    app.post("/api/appointments/:id/resend-confirmation", asyncHandler(async (req, res) => {
        const appointmentId = parseInt(req.params.id);
        const context = getLogContext(req);
        ...
        if (sendEmailRequested && client.emailAppointmentReminders && client.email) {
          ...
          `
                ...
                LoggerService.error("Failed to resend email confirmation", { ...context, appointmentId }, error);
            }
        }
        ...
        if (sendSmsRequested && client.smsAppointmentReminders && client.phone) {
            try {
                ...
                LoggerService.error("Failed to resend SMS confirmation", { ...context, appointmentId }, error);
            }
        }
    ...
    app.post("/api/appointments/resend-confirmation/:id", asyncHandler(async (req, res) => {
        const appointmentId = parseInt(req.params.id);
        const context = getLogContext(req);
        const { channel } = (req.body || {});
        ...
        if (sendEmailRequested && client.emailAppointmentReminders && client.email) {
          ...
          `
                ...
                LoggerService.error("Failed to resend email confirmation (alias)", { ...context, appointmentId }, error);
            }
        }
        if (sendSmsRequested && client.smsAppointmentReminders && client.phone) {
            try {
                ...
                LoggerService.error("Failed to resend SMS confirmation (alias)", { ...context, appointmentId }, error);
            }
        }
        if (!emailSent && !smsSent) {
            return res.status(400).json({
    ...
    app.post("/api/debug/test-email-functionality", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.get("/api/debug/test-data-retrieval/:clientId/:staffId/:serviceId", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
