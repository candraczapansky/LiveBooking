...
import { ValidationError, NotFoundError, ConflictError, asyncHandler } from "../utils/errors.js";
import LoggerService, { getLogContext } from "../utils/logger.js";
import { validateRequest } from "../middleware/error-handler.js";
import { invalidateCache } from "../utils/cache.js";
export function registerServiceRoutes(app, storage) {
    // Get all services
    app.get("/api/services", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
        const { category, categoryId, active, staffId, locationId } = req.query;
        ...
        if (filterCategoryId && !locationId && !staffId) {
            const categoryServices = await storage.getServicesByCategory(parseInt(filterCategoryId));
            // Add category information for consistency
            const categoryInfo = await storage.getServiceCategory(parseInt(filterCategoryId));
            ...
            const staffServices = await storage.getStaffServices(parseInt(staffId));
            // Get detailed service information for staff
            services = await Promise.all(staffServices.map(async (staffService) => {
            ...
            const staffAtLocation = allStaff.filter((s) => String(s.locationId) === String(locId));
            const staffIds = staffAtLocation.map((s) => s.id);
            const allAssignments = await storage.getAllStaffServices();
            const serviceIdsAtLocation = new Set(allAssignments
                .filter((a) => staffIds.includes(a.staffId))
                .map((a) => a.serviceId));
            const base = filterCategoryId
                ? await storage.getServicesByCategory(parseInt(filterCategoryId))
                : await storage.getAllServices();
            const filtered = base.filter((svc) => serviceIdsAtLocation.has(svc.id));
            services = await Promise.all(filtered.map(async (service) => {
    ...
    app.post("/api/services", asyncHandler(async (req, res) => {
        ...
        const { description: _desc, ...safeServiceData } = serviceData;
    ...
    // Update service
    app.put("/api/services/:id", validateRequest(insertServiceSchema.partial()), asyncHandler(async (req, res) => {
        const serviceId = parseInt(req.params.id);
        const context = getLogContext(req);
    ...
    app.get("/api/service-categories/:id", asyncHandler(async (req, res) => {
    ...
    app.post("/api/service-categories", validateRequest(insertServiceCategorySchema), asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.put("/api/service-categories/:id", validateRequest(insertServiceCategorySchema.partial()), asyncHandler(async (req, res) => {
        const categoryId = parseInt(req.params.id);
        const context = getLogContext(req);
        const updateData = req.body;
    ...
    app.delete("/api/service-categories/:id", asyncHandler(async (req, res) => {
        const categoryId = parseInt(req.params.id);
        const context = getLogContext(req);
    ...
    app.post("/api/staff/:staffId/services/:serviceId", asyncHandler(async (req, res) => {
    ...
    app.delete("/api/staff/:staffId/services/:serviceId", asyncHandler(async (req, res) => {
        const staffId = parseInt(req.params.staffId);
        const serviceId = parseInt(req.params.serviceId);
        const context = getLogContext(req);
    ...
    app.get("/api/services/:id/availability", asyncHandler(async (req, res) => {
        const serviceId = parseInt(req.params.id);
        ...
        const availableSlots = await storage.getServiceAvailability(serviceId, date ? new Date(date) : new Date(), staffId ? parseInt(staffId) : undefined);
        res.json({
            service,
            availableSlots,
    ...
    app.get("/api/services/:id/add-on-bases", asyncHandler(async (req, res) => {
        const id = parseInt(req.params.id);
        const bases = await storage.getBaseServicesForAddOn(id);
        res.json({ addOnServiceId: id, baseServiceIds: bases });
    }));
    app.post("/api/services/:id/add-on-bases", asyncHandler(async (req, res) => {
        const id = parseInt(req.params.id);
        ...
        await storage.setBaseServicesForAddOn(id, baseServiceIds.map((n) => parseInt(n)));
        res.json({ success: true });
    }));
    app.post("/api/services/:id/add-on-bases/:baseId", asyncHandler(async (req, res) => {
        const id = parseInt(req.params.id);
        const baseId = parseInt(req.params.baseId);
        await storage.addBaseServiceToAddOn(id, baseId);
        res.json({ success: true });
    }));
    app.delete("/api/services/:id/add-on-bases/:baseId", asyncHandler(async (req, res) => {
        const id = parseInt(req.params.id);
        const baseId = parseInt(req.params.baseId);
    ...
    app.put("/api/services/bulk-update", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
    ...
    app.post("/api/services/import", asyncHandler(async (req, res) => {
        const context = getLogContext(req);
