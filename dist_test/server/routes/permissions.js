...
import { permissions as permissionsTable, permissionGroups as permissionGroupsTable, permissionGroupMappings as permissionGroupMappingsTable, userPermissionGroups as userPermissionGroupsTable, userDirectPermissions as userDirectPermissionsTable, } from "../../shared/schema.js";
export function registerPermissionRoutes(app) {
    console.log("🔐 Registering permission routes");
    // Get all permissions
    app.get("/api/permissions", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
    ...
    app.get("/api/permission-groups", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
            res.set("Cache-Control", "no-store");
            // Fetch all groups
            ...
            const mappingRows = await db
                ...
                .leftJoin(permissionsTable, eq(permissionGroupMappingsTable.permissionId, permissionsTable.id));
            // Group permissions by groupId
            const groupIdToPermissions = {};
            for (const row of mappingRows) {
                if (!row.id)
                    continue; // skip if no permission joined
    ...
    app.post("/api/permission-groups", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
    ...
    app.put("/api/permission-groups/:id", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
    ...
    app.delete("/api/permission-groups/:id", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
            const groupId = parseInt(req.params.id);
            await db.delete(permissionGroupMappingsTable)
                .where(eq(permissionGroupMappingsTable.groupId, groupId));
    ...
    app.get("/api/users/:id/permissions", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
            res.set("Cache-Control", "no-store");
            ...
            const userGroups = await db
                ...
                .leftJoin(permissionGroupsTable, eq(userPermissionGroupsTable.groupId, permissionGroupsTable.id))
            ...
            const directPermRows = await db
                ...
                .leftJoin(permissionsTable, eq(userDirectPermissionsTable.permissionId, permissionsTable.id))
            ...
            for (const dp of directPermRows) {
                ...
                if (!permName)
                    continue;
                if (dp.isGranted)
                    permissionNameSet.add(permName);
                else
                    permissionNameSet.delete(permName);
            }
            res.json({
                success: true,
                data: {
    ...
    app.post("/api/users/:id/permission-groups", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
            const userId = parseInt(req.params.id);
    ...
    app.delete("/api/users/:id/permission-groups/:groupId", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
            const userId = parseInt(req.params.id);
            const groupId = parseInt(req.params.groupId);
            await db.delete(userPermissionGroupsTable)
                .where(and(eq(userPermissionGroupsTable.userId, userId), eq(userPermissionGroupsTable.groupId, groupId)));
            res.json({
                success: true,
    ...
    app.post("/api/users/:id/direct-permissions", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
            const userId = parseInt(req.params.id);
            ...
            const existing = await db
                ...
                .from(userDirectPermissionsTable)
                .where(and(eq(userDirectPermissionsTable.userId, userId), eq(userDirectPermissionsTable.permissionId, Number(permissionId))));
            if (existing[0]?.id) {
                await db
    ...
    app.delete("/api/users/:id/direct-permissions/:permissionId", authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
        try {
            const userId = parseInt(req.params.id);
            const permissionId = parseInt(req.params.permissionId);
            await db
                .delete(userDirectPermissionsTable)
                .where(and(eq(userDirectPermissionsTable.userId, userId), eq(userDirectPermissionsTable.permissionId, permissionId)));
