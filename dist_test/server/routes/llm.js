...
export function registerLLMRoutes(app, storage) {
    ...
    app.post("/api/llm/test", async (req, res) => {
        try {
            ...
            const context = {
                availableServices: services.map((s) => ({ name: s.name, description: s.description || undefined })),
                availableStaff: staff.map((st) => ({
                    name: st.user ? `${st.user.firstName || ""} ${st.user.lastName || ""}`.trim() : "Staff",
                    title: st.title || undefined,
                })),
