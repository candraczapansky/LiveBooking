...
import { appointmentConfirmationTemplate, appointmentReminderTemplate, followUpTemplate, birthdayTemplate, generateEmailHTML, generateEmailText, generateRawMarketingEmailHTML, htmlToText } from '../email-templates.js';
...
let storageInstance;
export function initializeEmailMarketingRoutes(storage) {
...
router.get('/analytics', async (req, res) => {
    try {
        ...
        const start = startDate ? new Date(startDate) : addDays(new Date(), -30);
...
router.post('/send-template', async (req, res) => {
    try {
        const { templateType, recipientEmail, recipientName, data, subject } = req.body;
        ...
        const contentLooksFull = !!(data && typeof data === 'object' && typeof data.content === 'string' && /<!DOCTYPE|<html|<body/i.test(data.content));
        const html = contentLooksFull
            ? generateRawMarketingEmailHTML(data.content, data.unsubscribeUrl || '#')
...
router.post('/bulk/promotional', async (req, res) => {
    try {
        const { recipientIds, subject, content, htmlContent, ctaButton, ctaUrl, specialOffer, promoCode } = req.body;
