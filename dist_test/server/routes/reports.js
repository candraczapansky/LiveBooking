...
const canAccessStaffData = (user, staffId) => {
    if (user.role === 'admin')
        return true;
...
export function registerReportRoutes(app, storage) {
    ...
    app.get("/api/reports/appointments/no-shows", async (req, res) => {
        ...
        `;
            ...
            const clientNoShows = result.rows.filter((row) => row.analysis_type === 'client');
            const staffNoShows = result.rows.filter((row) => row.analysis_type === 'staff');
            res.json({
                success: true,
                data: {
                    ...
                    summary: {
                        total_no_shows: result.rows.reduce((sum, row) => sum + parseInt(row.no_show_count), 0),
    ...
    app.get("/api/reports/strategic-insights", async (req, res) => {
      ...
      `;
            ...
            const topService = topServiceResult.rows[0];
            if (topService) {
                insights.push({
                    type: 'info',
