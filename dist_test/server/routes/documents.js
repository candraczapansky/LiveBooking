...
export function registerDocumentsRoutes(app, storage) {
    ...
    app.put("/api/documents/:id", asyncHandler(async (req, res) => {
        ...
        if (!parsed.success)
            throw new ValidationError("Invalid document update");
    ...
    app.post("/api/documents/:id/send-sms", asyncHandler(async (req, res) => {
        const id = req.params.id;
        ...
        const { clientId, phone, customMessage } = parsed.data;
        let destinationPhone;
        let resolvedClientId;
        if (clientId) {
            const user = await storage.getUser(Number(clientId));
            if (!user || !user.phone)
                throw new ValidationError("Selected client does not have a phone number");
        ...
        if (!destinationPhone)
            throw new ValidationError("No destination phone number provided");
        // Build public URL using request host to match the exact frontend origin
        const xfProto = req.headers['x-forwarded-proto'] || req.protocol || 'https';
        const xfHost = req.headers['x-forwarded-host'] || req.get('host');
        const baseFromReq = xfHost ? `${xfProto}://${xfHost}` : undefined;
        const basePath = `/api/public-documents/${id}`;
        const publicUrl = baseFromReq
            ? `${baseFromReq}${basePath}${resolvedClientId != null ? `?clientId=${resolvedClientId}` : ''}`
    ...
    app.post("/api/documents/:id/send-email", asyncHandler(async (req, res) => {
        ...
        if (!parsed.success)
            throw new ValidationError("Invalid request payload");
        const { clientId, email, subject, customMessage } = parsed.data;
        let destinationEmail = email;
        let resolvedClientId;
        if (clientId) {
            const user = await storage.getUser(Number(clientId));
            if (!user || !user.email)
                throw new ValidationError("Selected client does not have an email address");
            destinationEmail = user.email;
            resolvedClientId = Number(clientId);
        }
        if (!destinationEmail)
            throw new ValidationError("No destination email provided");
        const xfProto = req.headers['x-forwarded-proto'] || req.protocol || 'https';
        const xfHost = req.headers['x-forwarded-host'] || req.get('host');
        const baseFromReq = xfHost ? `${xfProto}://${xfHost}` : undefined;
        const basePath = `/api/public-documents/${id}`;
    ...
    app.get("/api/public-documents/:id", asyncHandler(async (req, res) => {
        const id = req.params.id;
        const row = await storage.getSystemConfig(`document:${id}`);
        if (!row)
            throw new NotFoundError("Document");
