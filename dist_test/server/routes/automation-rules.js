...
export function registerAutomationRuleRoutes(app, storage) {
    // List all automation rules
    app.get("/api/automation-rules", asyncHandler(async (_req, res) => {
        const rules = await storage.getAllAutomationRules();
        res.json(rules);
    }));
    // Create a new automation rule
    app.post("/api/automation-rules", asyncHandler(async (req, res) => {
        const data = insertAutomationRuleSchema.parse(req.body);
        const created = await storage.createAutomationRule(data);
        res.status(201).json(created);
    }));
    // Update an automation rule
    app.put("/api/automation-rules/:id", asyncHandler(async (req, res) => {
        const id = parseInt(req.params.id);
        if (Number.isNaN(id)) {
            return res.status(400).json({ error: "Invalid rule id" });
        }
        const updateData = updateAutomationRuleSchema.parse(req.body);
        const updated = await storage.updateAutomationRule(id, updateData);
        if (!updated) {
    ...
    app.delete("/api/automation-rules/:id", asyncHandler(async (req, res) => {
        const id = parseInt(req.params.id);
        if (Number.isNaN(id)) {
            return res.status(400).json({ error: "Invalid rule id" });
        }
    ...
    app.post("/api/automation-rules/trigger", asyncHandler(async (req, res) => {
        ...
        }).refine(v => (typeof v.appointmentId === 'number' && !Number.isNaN(v.appointmentId)) || !!v.testEmail, {
        ...
        if (testEmail && (appointmentId == null || Number.isNaN(Number(appointmentId)))) {
            ...
            await triggerAutomations(resolvedTrigger, {
                id: 0,
                clientId: 0,
                serviceId: 0,
                staffId: 0,
        ...
        await service.triggerAutomations(resolvedTrigger, context, customTriggerName);
