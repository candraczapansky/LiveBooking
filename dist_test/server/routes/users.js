import { insertUserColorPreferencesSchema, updateUserSchema, insertClientSchema } from "../../shared/schema.js";
...
export function registerUserRoutes(app, storage) {
    ...
    app.get("/api/clients/:id/form-submissions", async (req, res) => {
        try {
            ...
            if (!storage.getClientFormSubmissions) {
                return res.status(501).json({ error: "Form submissions not supported" });
            }
            const submissions = await storage.getClientFormSubmissions(id);
    ...
    app.post("/api/clients/import", async (req, res) => {
        try {
            ...
            try {
                ...
                existingEmails = new Set(allUsers.map((u) => (u.email || '').toLowerCase()).filter(Boolean));
            }
            catch (e) {
                // If fetching all users fails, continue without pre-cache; rely on DB unique constraints
            }
            const generateFallbackEmail = (firstName, lastName, index) => {
            ...
            let currentIndex = 0;
            async function processOne(raw, index) {
    ...
    app.get("/api/users", async (req, res) => {
        try {
            ...
            if (safeUsers.length > 0) {
                ...
                const usersWithPhones = safeUsers.filter((u) => u.phone && u.phone.trim() !== '');
    ...
    app.get("/api/users/:id", async (req, res) => {
        try {
            ...
            const safeUser = {
                ...
                emailAccountManagement: user.emailAccountManagement ?? true,
                emailAppointmentReminders: user.emailAppointmentReminders ?? true,
                emailPromotions: user.emailPromotions ?? false,
                smsAccountManagement: user.smsAccountManagement ?? false,
                smsAppointmentReminders: user.smsAppointmentReminders ?? true,
                smsPromotions: user.smsPromotions ?? false,
