...
export function createRateLimit(options) {
    return rateLimit({
        ...
        keyGenerator: options.keyGenerator || ((req) => ipKeyGenerator(req.ip || req.connection.remoteAddress || 'unknown')),
        handler: (req, res) => {
...
export function securityLogging(req, res, next) {
    ...
    const isSuspicious = suspiciousPatterns.some(pattern => pattern.test(req.url) || pattern.test(userAgent));
    if (isSuspicious) {
