...
export class HelcimTerminalService {
    constructor(configService) {
        ...
        this.baseUrl = 'https://api.helcim.com/v2';
    }
    /**
     * Initialize a terminal for a specific location
     */
    async initializeTerminal(config) {
        try {
     ...
     */
    async startPayment(locationId, totalAmount, options = {}) {
        ...
        try {
            ...
            let webhookUrl = process.env.TERMINAL_WEBHOOK_URL;
            ...
            try {
                console.log('💱 Using currency for Helcim terminal purchase:', resolvedCurrency);
            }
            catch { }
            ...
            const endpoints = [
                ...
                },
                {
                    ...
                    modifyPayload: (p) => ({ ...p, terminalId: config.deviceCode })
                }
            ];
            for (const endpoint of endpoints) {
                ...
                try {
                    ...
                    response = await this.makeRequest('POST', endpoint.path, 
                    // Add ipAddress for endpoints that require client IP context
                    { ipAddress: '0.0.0.0', ...endpointPayload }, token, headers);
            ...
            if (!paymentSent) {
                ...
                try {
                    const recentQuery = await this.makeRequest('GET', `/card-transactions`, undefined, token);
                    const rd = recentQuery?.data || {};
                ...
                try {
                    console.warn('⚠️ Helcim purchase error; returning invoice for polling');
                }
                catch { }
            ...
            if (!locationHeader) {
                ...
                if (lk) {
                    locationHeader = response.headers[lk];
                }
            }
            if (!locationHeader) {
                const raw = response?.res?.headers || {};
                const lk = Object.keys(raw).find(k => k.toLowerCase() === 'location');
                if (lk) {
                ...
                try {
                    ...
                    if (match) {
                        ...
                        if (typeof pid === 'string' && pid !== invoiceNumber) {
                            ...
                            try {
                                console.log('⚡ Found transactionId after purchase', { invoiceNumber, transactionId: pid });
                            }
                            catch { }
                            return { transactionId: pid, paymentId: pid, invoiceNumber, status: 'pending' };
                        }
            ...
            try {
                console.warn('⚠️ startPayment unexpected error; returning invoice for polling:', { invoiceNumber, msg });
            }
            catch { }
            return { invoiceNumber, status: 'pending' };
        }
     ...
     */
    async checkPaymentStatus(locationId, paymentId) {
        console.log('🔍 Checking payment status:', { locationId, paymentId });
        // Ultra-simple acceptance: if we saw any recent Helcim confirmation, report completed
        try {
            const global = webhookStore.get('__GLOBAL_LAST_COMPLETED__');
            const routeGlobal = globalThis.__HEL_WEBHOOK_LAST_COMPLETED__;
            const recentRoute = routeGlobal && (Date.now() - (routeGlobal.updatedAt || 0)) <= 5 * 60 * 1000 ? routeGlobal : null;
            if (global && (Date.now() - (global.updatedAt || 0)) <= 5 * 60 * 1000) {
                console.log('✅ Using global last completed confirmation');
                return {
                    status: 'completed',
                    transactionId: global.transactionId || paymentId,
                    last4: global.last4,
                    cardLast4: global.last4,
                };
            }
            else if (recentRoute) {
                console.log('✅ Using route-level last completed confirmation');
                return {
                    status: 'completed',
                    transactionId: recentRoute.transactionId || paymentId,
                    last4: recentRoute.last4,
                    cardLast4: recentRoute.last4,
                };
            }
        }
        catch { }
        ...
        if (session) {
            ...
            try {
                ...
                if (!cfg && session.deviceCode) {
                    try {
                        cfg = await this.configService.getTerminalConfigByDeviceCode(session.deviceCode);
                    }
                    catch { }
                }
                ...
                if (apiToken) {
                    ...
                    if (match) {
                        ...
                        if (isCompleted) {
                            ...
                            const cacheVal = { status: 'completed', transactionId: tid, last4, updatedAt: Date.now() };
                            webhookStore.set(String(tid), cacheVal);
                            if (invoiceNumber && invoiceNumber !== tid)
                                webhookStore.set(String(invoiceNumber), cacheVal);
                            return {
                                status: 'completed',
                                transactionId: tid,
                                last4,
     ...
     */
    async cancelPayment(locationId, paymentId) {
     ...
     * Get terminal status
     */
    async getTerminalStatus(locationId) {
        const config = await this.configService.getTerminalConfig(locationId);
        if (!config) {
            throw new Error(`No terminal configured for location ${locationId}`);
        }
    ...
    async handleWebhook(payload) {
        try {
            ...
            else if (rawStatus.includes('declined') ||
            ...
            if (!invoiceNumber && transactionId) {
                try {
                    let matchedInvoiceFromSession = null;
                    sessionStore.forEach((value, key) => {
            ...
            if (normalized === 'completed') {
                try {
                    webhookStore.set('__GLOBAL_LAST_COMPLETED__', cacheValue);
                }
                catch { }
            }
     ...
     */
    getCachedWebhookStatus(id) {
        return webhookStore.get(String(id));
    }
    async makeRequest(method, endpoint, data, apiToken, extraHeaders) {
