...
export function registerExternalRoutes(app, storage) {
    ...
    app.get("/api/external/staff-availability", optionalApiKey, async (req, res) => {
        try {
            ...
            const staffWithDetails = await Promise.all(allStaff.map(async (staff) => {
                ...
                const serviceDetails = await Promise.all(services.map(async (staffService) => {
            ...
            const result = staffId
                ? staffWithDetails.filter(staff => staff.id === parseInt(staffId))
                : staffWithDetails;
    ...
    app.get("/api/external/services", optionalApiKey, async (req, res) => {
        try {
            ...
            const servicesWithDetails = await Promise.all(allServices.map(async (service) => {
                ...
                const assignedStaff = await Promise.all(staffAssignments.map(async (assignment) => {
            ...
            if (categoryId) {
                filteredServices = filteredServices.filter(service => service.category?.id === parseInt(categoryId));
            }
            if (staffId) {
                filteredServices = filteredServices.filter(service => service.assignedStaff.some(assignment => assignment.staffId === parseInt(staffId)));
            }
    ...
    app.get("/api/external/appointments", validateApiKey, async (req, res) => {
        try {
            ...
            if (staffId) {
                ...
                if (!Number.isNaN(sid)) {
                    appointments = appointments.filter((a) => a.staffId === sid);
                }
            }
            // Filter by location
            if (locationId) {
                ...
                if (!Number.isNaN(lid)) {
                    appointments = appointments.filter((a) => a.locationId === lid);
                }
            }
            // Filter by status
            if (status) {
                const st = String(status);
                appointments = appointments.filter((a) => a.status === st);
            }
    ...
    app.post("/api/appointments/webhook", validateApiKey, async (req, res) => {
        try {
            ...
            const { clientId, serviceId, staffId, startTime, endTime, notes, status = 'confirmed', externalAppointmentId, clientInfo, serviceInfo, staffInfo } = validationResult.data;
            ...
            if (finalStaffId) {
                ...
                const blockedSchedules = staffSchedules.filter((schedule) => schedule.dayOfWeek === dayName &&
