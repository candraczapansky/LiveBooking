...
export async function seedPermissions(storage) {
    ...
    try {
        ...
        for (const permission of DEFAULT_PERMISSIONS) {
            const existingPermission = await storage.getPermissionByName?.(permission.name) ?? undefined;
        ...
        for (const group of DEFAULT_PERMISSION_GROUPS) {
            const existingGroup = await storage.getPermissionGroupByName?.(group.name) ?? undefined;
            if (!existingGroup) {
