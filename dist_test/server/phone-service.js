...
export class PhoneService {
    ...
    static async makeOutboundCall(toNumber, staffId, userId, appointmentId, purpose = 'outbound') {
        try {
            if (!client) {
    ...
    static async handleIncomingCall(callSid, fromNumber, toNumber) {
        try {
    ...
    static async updateCallStatus(callSid, status, duration) {
        try {
    ...
    // Save call recording
    static async saveCallRecording(callSid, recordingSid, recordingUrl, duration) {
        try {
    ...
    static async getCallHistoryForUser(userId) {
        try {
    ...
    static async getRecentCalls(limit = 50) {
        try {
    ...
    // Add notes to a call
    static async addCallNotes(callId, notes, staffId) {
        try {
            await db
                .update(phoneCalls)
                .set({ notes, staffId })
    ...
    static async updateCallUserAssociation(callId, userId) {
        try {
            await db
                .update(phoneCalls)
                .set({ userId })
    ...
    static async getRecordingDownloadUrl(recordingSid) {
        try {
            if (!client) {
    ...
    static async getCallAnalytics(startDate, endDate) {
        try {
            let conditions = [];
            if (startDate && endDate) {
                conditions.push(and(gte(phoneCalls.createdAt, startDate), lte(phoneCalls.createdAt, endDate)));
            }
