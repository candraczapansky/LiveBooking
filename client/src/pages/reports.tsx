...
const SpecificReportView = ({ reportType, timePeriod }: { reportType: string; timePeriod: string }) => {
  ...
  // Calculate stats based on timePeriod
  ...
  const totalProfit = totalRevenue * 0.7; // Estimate 70% profit margin
  const totalClients = (users as any[]).filter((user: any) => user.role === "client").length;
      ...
      <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ${
        ...
        <main className="flex-1 overflow-y-auto bg-gray-50 dark:bg-gray-900 p-4 md:p-6">
          <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    {selectedReport ? getReportTitle(selectedReport) : "Reports"}
                  </h1>
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                    {selectedReport 
                      ? getReportDescription(selectedReport)
                      : "Choose a report category to view detailed analytics and insights"
                    }
                  </p>
                </div>
              </div>
            ...
            {/* Show Landing Page or Specific Report */}
            {!selectedReport ? (
